...
app.put('/api/user/preferences', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const { split_type, difficulty_level, day_override, gemini_api_key, reps_style, reps_min, reps_max,
            exercises_per_muscle, sets_per_exercise, rest_seconds, include_bodyweight, focused_muscle } = req.body;
        ...
        await db.query(`
            ...
            SET split_type = COALESCE(?, split_type), 
                ...
                exercises_per_muscle = COALESCE(?, exercises_per_muscle),
                sets_per_exercise = COALESCE(?, sets_per_exercise),
                rest_seconds = COALESCE(?, rest_seconds),
                include_bodyweight = COALESCE(?, include_bodyweight),
                focused_muscle = COALESCE(?, focused_muscle)
            WHERE user_id = ?
        `, [split_type, difficulty_level, day_override, gemini_api_key, reps_style, reps_min, reps_max, exercises_per_muscle, sets_per_exercise, rest_seconds, include_bodyweight, focused_muscle, userId]);
